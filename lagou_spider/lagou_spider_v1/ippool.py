...
class IpSpiders():
    ...
    def spider_one(self):
    # 爬虫A
        web_str= self.download_page(url=r"http://www.kuaidaili.com/free/inha/1")

        cprint(web_str)

        # 数据格式: data=[{'ip':'xx.xx.xx.xx','port':'80','location','time'},{'ip':'xx.xx.xx.xx','port':'80'}]
        data = []
        one_ip_data_dict = {'ip':None,'port':None,'location':None,'time':None}
...
# 每个网站对饮给一个方法爬虫
    ...
    def spider_three(self):
        ...
        # 爬虫C
        web_str= self.download_page(url=r"http://www.xdaili.cn/ipagent/freeip/getFreeIps?page=1&rows=10")
        cprint(web_str)
